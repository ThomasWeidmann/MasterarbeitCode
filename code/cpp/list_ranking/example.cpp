...
class example
{
	...
	/*
		...
		*/
		auto queue = message_queue::make_buffered_queue<std::tuple<std::uint64_t,std::uint64_t,std::uint32_t>>(MPI_COMM_WORLD, [](auto& buf, message_queue::PEID receiver) {});
		...
		auto on_message = [&](message_queue::Envelope<std::tuple<std::uint64_t,std::uint64_t,std::uint32_t>> auto envelope) {
			for (int i = 0; i < envelope.message.size(); i++)
			{
				std::uint64_t ruler_source =std::get<0>(envelope.message[i]);
				std::uint64_t destination = std::get<1>(envelope.message[i]);
				std::uint32_t distance = std::get<2>(envelope.message[i]);
				std::uint32_t local_index = destination - node_offset;
				...
				{
					std::uint32_t targetPE = calculate_targetPE(s[destination - node_offset]);
				...
				else
				{
					...
					std::uint32_t targetPE = calculate_targetPE(s[ruler_index]);
